...
test('config bounds and explicit keybinding overrides',()=>{assert.equal(parseBackgroundWorkConfig({}).enabled,true);assert.equal(parseBackgroundWorkConfig({}).bashTool,'wrap');assert.equal(parseBackgroundWorkConfig({}).subagents,true);assert.equal(parseBackgroundWorkConfig({bashTool:'off',subagents:false}).bashTool,'off');assert.equal(parseBackgroundWorkConfig({subagents:false}).subagents,false);assert.equal(parseBackgroundWorkConfig({}).statusIndicator,true);assert.equal(parseBackgroundWorkConfig({statusIndicator:false}).statusIndicator,false);assert.equal(parseBackgroundWorkConfig({maxOutputLines:-1}).maxOutputLines,2000);const d=fs.mkdtempSync(path.join(os.tmpdir(),'bg-'));const p=path.join(d,'keys.json');fs.writeFileSync(p,JSON.stringify({'tui.editor.cursorLeft':['left']}));assert.equal(detectShortcutConflict('ctrl+b',p),undefined);fs.writeFileSync(p,JSON.stringify({foo:['ctrl+shift+b']}));assert.equal(detectShortcutConflict('ctrl+shift+b',p),'foo')});
