...
test('acknowledgement racing a queued retry drains every delivery record',async()=>{
 const store=createBackgroundWorkStore(),manager=new BackgroundJobManager(store);manager.setSession({sessionId:'s1'});const item=fake('retry-ack');manager.register(item.execution);manager.promoteAll();item.finish();await tick();
 manager.markQueued(['retry-ack']);assert.equal(manager.retryQueued('retry-ack'),true);manager.markDelivered(['retry-ack']);
 assert.equal(store.pendingDeliveries.length,0);assert.equal(store.queued.has('retry-ack'),false);assert.equal(store.queuedDeliveries.has('retry-ack'),false);assert.equal(store.delivered.has('retry-ack'),true);
});
test('well-formed acknowledgement cannot pre-deliver a running job',async()=>{
 const store=createBackgroundWorkStore(),manager=new BackgroundJobManager(store);manager.setSession({sessionId:'s1'});const item=fake('future-delivery');manager.register(item.execution);manager.promoteAll();manager.markDelivered(['future-delivery']);assert.equal(store.delivered.has('future-delivery'),false);item.finish();await tick();assert.equal(store.pendingDeliveries.length,1);
});
...
test('concurrent cancellation requests share one adapter cancellation',async()=>{
 const store=createBackgroundWorkStore(),manager=new BackgroundJobManager(store,{},1_000);manager.setSession({sessionId:'s1'});const item=fake('duplicate-cancel');manager.register(item.execution);manager.promoteAll();
 const first=manager.cancel('duplicate-cancel'),second=manager.cancel('duplicate-cancel');await tick();item.finish('cancelled');assert.deepEqual(await Promise.all([first,second]),[true,true]);assert.equal(item.cancels,1);
});
test('config bounds and explicit keybinding overrides',()=>{assert.equal(parseBackgroundWorkConfig({}).enabled,true);assert.equal(parseBackgroundWorkConfig({}).bashTool,'wrap');assert.equal(parseBackgroundWorkConfig({}).subagents,true);assert.equal(parseBackgroundWorkConfig({bashTool:'off',subagents:false}).bashTool,'off');assert.equal(parseBackgroundWorkConfig({subagents:false}).subagents,false);assert.equal(parseBackgroundWorkConfig({}).statusIndicator,true);assert.equal(parseBackgroundWorkConfig({statusIndicator:false}).statusIndicator,false);assert.equal(parseBackgroundWorkConfig({maxOutputLines:-1}).maxOutputLines,2000);assert.equal(parseBackgroundWorkConfig({}).completionBehavior,'adaptive');assert.equal(parseBackgroundWorkConfig({completionBehavior:'notify-and-resume'}).completionBehavior,'notify-and-resume');assert.equal(parseBackgroundWorkConfig({completionBehavior:'notify-only'}).completionBehavior,'notify-only');assert.equal(parseBackgroundWorkConfig({completionBehavior:'junk'}).completionBehavior,'adaptive');assert.equal(parseBackgroundWorkConfig({}).promotionYield,'interrupt');assert.equal(parseBackgroundWorkConfig({promotionYield:'steer'}).promotionYield,'steer');assert.equal(parseBackgroundWorkConfig({promotionYield:'off'}).promotionYield,'off');assert.equal(parseBackgroundWorkConfig({promotionYield:'junk'}).promotionYield,'interrupt');const d=fs.mkdtempSync(path.join(os.tmpdir(),'bg-'));const p=path.join(d,'keys.json');fs.writeFileSync(p,JSON.stringify({'tui.editor.cursorLeft':['left']}));assert.equal(detectShortcutConflict('ctrl+b',p),undefined);assert.equal(detectShortcutConflict('esc',p),'app.interrupt');assert.equal(detectShortcutConflict('return',p),'tui.input.submit');assert.equal(detectShortcutConflict('ctrl+shift+p',p),'app.model.cycleBackward');assert.equal(detectShortcutConflict('a',p),'invalid shortcut');fs.writeFileSync(p,JSON.stringify({foo:['shift+ctrl+b']}));assert.equal(detectShortcutConflict('ctrl+shift+b',p),'foo')});
...
test('adaptive completion resumes only when its policy allows it',()=>{
 const run=(resume:boolean)=>{const sent:any[]=[];const store={pendingDeliveries:[{jobId:'adaptive',status:'succeeded' as const,finishedAt:2,durationMs:1,summary:'done'}],delivered:new Set<string>(),queued:new Set<string>()};const delivery=new CompletionDelivery({sendMessage:(message,options)=>sent.push([message,options])},store,{debounceMs:0,maxOutputBytes:1024,maxOutputLines:10,completionBehavior:'adaptive',shouldResume:ids=>{assert.deepEqual(ids,['adaptive']);return resume},onQueued:ids=>ids.forEach(id=>store.queued.add(id)),onError:error=>{throw error}});delivery.flush();return sent[0]};
 const resumed=run(true),notResumed=run(false);assert.equal(resumed[1].triggerTurn,true);assert.equal(resumed[0].details.autoResume,true);assert.equal(notResumed[1].triggerTurn,false);assert.equal(notResumed[0].details.autoResume,false);
});
...
test('cancellation deadline bounds never-resolving and rejecting adapters',async()=>{
 ...
 const started=Date.now();const result=await m.cancelAll();assert.equal(result.failed.length,1);assert.ok(Date.now()-started<200);assert.equal(m.allSnapshots().find((item)=>item.jobId==='never')?.state,'background-running');
